...
const MyJobs = () => {
  const email = "kanchansingh30nov@gmail.com";
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch("http://localhost:3000/myJobs/kanchansingh30nov@gmail.com")
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        setIsLoading(false);
  ...
  const handleDelete = (id) => {
    ...
    .then((result) => {
        ...
        if (result.acknowledged === true) {
            ...
            setIsLoading(!isLoading); // This line reloads the jobs after deletion
        }
    })
    .catch((error) => {
      ...
      <div className="my-job-container">
        ...
        <div className="search-box p-2 text-center  mb-2 flex">
          <input
            ...
            name="search"
            id="search"
            className="py-2 pl-3 border focus:outline-nonelg:w-6/12  mb-4 w-full"
          />
          <button
            className="bg-blue-600 rounded-sm text-white cursor-pointer mb-4  px-8 py-2 font-semibold "
                  ...
                  isLoading?(<h4 className="flex items-center justify-center h-20"><p>Loading....</p></h4>):( <tbody>
                    {
                      ...
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                        <button className="" ><Link to={`/edit-job/${job?._id}`}>Edit</Link></button>
                      </td>
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
